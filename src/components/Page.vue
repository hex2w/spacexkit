...
function setPage(newValue) {
  console.log(`SET 'page' TO NEW VALUE => ${newValue}\nOLD VALUE => ${page.value}`)
  page.value = newValue
  router.push({
    path: route.fullPath,
...
// function nextPage() { page.value += 1; router.push({path:route.fullPath, query: { page }}) }
// function prevPage() { page.value -= 1;  }

onMounted(() => {
  setTimeout(_ => { delayElapsed.value = true }, 200)
    functions.get(`${route.path}`, query)
...
<template>
  ...
  <div v-if="!loading && delayElapsed && !data.error">
    ...
    <div v-else>
      <div class="p-2 md:p-4" v-for="(document, index) in data.docs" :key="index">
        <PageItem :data="document" />
      </div>

      <div v-if="data.totalPages != 1" class="p-2 md:p-4 lg:p-6 fixed bottom-0 font-semibold bg-white text-black rounded-t-2xl border-t-4 border-cyan-400 shadow-md shadow-cyan-400">
        <div class="mx-auto inline-flex flex-wrap space-x-6 max-w-xl md:max-w-2xl lg:max-w-3xl">
          ...
          <button :disabled="!data.hasPrevPage" @click="prevPage()"
            class="bg-teal-500 rounded-full hover:bg-teal-400 disabled:bg-slate-400">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
          </button>
          <button :disabled="!data.hasNextPage" @click="nextPage()"
            class="ring ring-teal-500 hover:ring-teal-400 bg-teal-500 rounded-full hover:bg-teal-400 disabled:ring-slate-400 disabled:bg-slate-400">
